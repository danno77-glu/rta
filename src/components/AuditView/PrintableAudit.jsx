...
const PrintableAudit = ({ audit, damageRecords, onClose, auditorDetails }) => {
  ...
  const [pdfBlobUrl, setPdfBlobUrl] = useState(null); // State to hold the PDF blob URL
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      setPdfBlobUrl(url); // Set the blob URL for the download link


      // Try opening in a new tab/window FIRST
      const newWindow = window.open(url, '_blank');
      if (!newWindow) {
        // If popup blocked, indicate to the user and the link will be visible
        setPdfError("Popup blocked. Please use the download link below.");
      }
      ...
      <div className="preview-header no-print">
        ...
        <div className="preview-actions">
          ...
          {pdfBlobUrl && (
            <a
              href={pdfBlobUrl}
              download={`audit-${audit.reference_number}.pdf`}
